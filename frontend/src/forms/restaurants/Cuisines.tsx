        ...
        >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                {cuisineList.map(cusine => (
                   <label key={cusine}>
                     <input type="checkbox"  {...register("cuisines")} value={cusine} className=" h-[18px] w-[18px] mr-3 bg-slate-800 accent-black rounded-md" />
                     <span className=" font-medium">{cusine}</span>
                   </label>
                ))}
            </div>

            {errors.cuisines && <p className=" text-sm text-red-500 my-3">{errors.cuisines.message}</p>}
