...
import {  restAurentCreationFailure, restAurentCreationSatrt, restAurentCreationSuccess } from "@/redux/slices/userSlice";
  ...
  async function onSubmit(data: restaurentType) {
    ...
    Menu.forEach((Item, index) => {
      ...
      formData.append(
        ...
        (Item.price as unknown as number).toString()
      );
    });

    try {
      const url = preData
        ? `${baseUrl}/api/restaurants/update/${userId}`
        : `${baseUrl}/api/restaurants/create`;
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {loading ? (
            ...
            <button
              type="submit"
              disabled={loading}
              className=" disabled:opacity-90 text-xl font-semibold px-10 py-2 bg-orange-500 text-white rounded-md"
            >
