...
const Details = () => {
  const {
    register,
    formState: { errors },
  } = useFormContext<restaurentType>();
  return (
    <div className="space-y-2">
      <div className=" pb-6">
        <h3 className=" text-2xl font-semibold">Details</h3>
        <p className=" text-sm text-slate-500">
          Enter the Details About your Restaurants
        </p>
      </div>
      ...
      <div>
        <div className="mb-6">
          <label>
            ...
            <input
              className={`w-full bg-transparent p-4 border  rounded-md outline-none  focus:border-2 hover:border-slate-800 ${
                errors.restaurantName
                  ? "border-red-500 focus:border-red-500 hover:border-red-500"
                  : "border-slate-400 focus:border-blue-600"
              }`}
              {...register("restaurantName")}
            />
            {errors.restaurantName && (
              <span className=" block mt-1 text-sm text-red-500 mx-1 ">
